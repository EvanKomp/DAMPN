...
class TestDAMPLayer:
    ...
    def test_forward(self, DAMPLayer, BATCH):
        ...
        assert output[1].shape == (5, 2), "edge hidden state incorrect shape, should not have been updated"
        ...
        assert output[0].shape == (5, 8), "node hidden state incorrect shape"
        assert output[1].shape == (5, 3), "edge hidden state incorrect shape, should have been updated"
        return
    ...
    def test_backpropegate(self, DAMPLayer, BATCH):
        ...
        for key, params in initial_state.items():
            assert not torch.equal(params, final_state[key]), f"{key} params not updated"
